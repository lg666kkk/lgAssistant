...
describe("SemanticStore 语义召回", () => {
  ...
  beforeAll(async () => {
    if (!canRun) return;
    for (const m of seed) await store.set(m.key, m.content, { ns });
  });
  ...
  afterAll(async () => {
    if (!canRun) return;
    for (const m of seed) await store.forget(m.key);
    await store.forget(mutKey);
  });
  ...
  it.skipIf(!canRun)("get/list 不带 score（只有 recall 才有相似度）", async () => {
    const rec = await store.get(seed[0].key);
  ...
  it.skipIf(!canRun)("覆盖 set 后向量同步更新（同一条记忆按新语义召回）", async () => {
    ...
    await store.set(mutKey, "用户喜欢喝美式咖啡");
    const before = await store.recall("乐器演奏 弹钢琴", 10);
    const beforeScore = before.find((h) => h.key === mutKey)?.score ?? 0;

    // 覆盖成「弹钢琴」：upsert 会重算 embedding，向量应跟着变
    await store.set(mutKey, "用户在学习弹钢琴，每天练一小时");
    const after = await store.recall("乐器演奏 弹钢琴", 10);
