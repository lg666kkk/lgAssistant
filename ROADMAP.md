...
> 分两个层次：
> - **2A. 单 Agent 基础能力**（P0 优先级）— 单个 Agent 能调用工具、多步执行、完成任务
> - **2B. Harness 多 Agent 协作**（P2/P3 优先级）— 用于复杂、长期运行的任务，建立在 2A 之上

---

### 2A. 单 Agent 基础能力（P0 优先级）

这是整个系统的分水岭 — 从"聊天机器人"进化为"能干活的 Agent"。

#### 2A.0 Agent Runtime — 运行时基础设施
> 单个 Agent 的运行时容器，所有基础能力都挂载在它上面。

- Runtime 是包裹在 LLM 外面的运行时容器，负责：
  - **生命周期管理**：初始化 → 接收输入 → Prompt 组装 → LLM 调用 → 工具执行 → 输出响应 → 清理
  - **Tool Registry 挂载点**：所有工具（内置 / MCP / Plugin）统一注册
...
- Runtime 是可配置的：通过 Config 文件控制行为（允许哪些工具、沙箱范围、Hook 列表等）
- 后续所有能力（Tool、Loop、Memory、MCP）都是"插入 Runtime 的模块"
...
#### 2A.7 MCP 协议支持（Model Context Protocol）
...
> 基于 Anthropic 官方实践：用于复杂、长期运行的任务  
> 建立在 2A 单 Agent 基础能力之上  
> 参考：https://www.anthropic.com/engineering/harness-design-long-running-apps

**核心理念**：
- Harness = 让模型能够作为 Agent 行动起来的外循环系统
- 内循环（模型推理）：给定上下文 → 生成下一步
- 外循环（Harness）：决定什么时候开始新的内循环、给它什么上下文、如何验证输出、何时回退、何时停止
- 评估 Agent 效果时，评估的不是模型本身，而是 **model + harness 的组合**

#### 2B.1 Planner-Generator-Evaluator 三角架构
> 让多个专门化 Agent 协同工作，处理复杂任务。

**三个核心 Agent**：
- **Planner Agent（规划者）**：
  - 输入：用户需求
  - 输出：`spec.md`（产品规格）、`features.json`（200+ 条特性列表）
  - 职责：任务分解、优先级排序、成功标准定义

- **Generator Agent（生成者）**：
  - 输入：`spec.md`、`features.json`、`evaluation.md`（上一轮反馈）
  - 输出：代码、`implementation.md`（实现说明）
  - 职责：编写代码、运行测试、提交 git
  - 基于 2A 的所有工具能力（文件操作、终端命令、MCP 等）

- **Evaluator Agent（评估者）**：
  - 输入：`implementation.md`、`features.json`
  - 输出：`evaluation.md`（评估报告 + 改进建议）
  - 职责：交互式测试、按标准打分、发现缺陷
  - 关键：与 Generator 完全分离，避免自我评估偏差
...
- **初始化 Agent**：首次运行时设置环境
  - 创建 `init.sh`（启动脚本）
  - 创建 `claude-progress.txt`（进度记录）
  - 创建 `features.json`（特性列表，JSON 格式防止篡改）
  - 初始 git commit（展示项目结构）

- **编码 Agent**：后续会话增量进展
  - 标准工作流程：
    1. `pwd` 确认工作目录
    2. 读取 `git log` + `claude-progress.txt`
    3. 从 `features.json` 选择最高优先级未完成项
    4. 运行 `init.sh` 启动开发服务器
    5. 实现单个特性
    6. 提交代码 + 更新进度文件

**四大失败模式的预防**：
| 失败模式 | 初始化 Agent 的预防 | 编码 Agent 的约束 |
|---------|-------------------|------------------|
| 过早宣布完成 | 创建 200+ 条目的特性列表 | 一次只处理一个特性 |
| 留下未文档化的 bug | 初始 git 仓库 + 进度文件 | 每次会话结束时提交 + 更新 |
| 不充分的测试 | 特性列表文件 | 使用浏览器自动化工具验证 |
| 启动困难 | 编写 `init.sh` 脚本 | 会话开始时运行脚本 |

#### 2B.3 上下文管理策略

**上下文重置而非压缩**：
- 原因：模型会"上下文焦虑"，接近认知极限时提前结束
- 方案：每个新会话完全重置上下文窗口
- 状态传递：通过文件（`claude-progress.txt`、git log）

**文件作为状态传递机制**：
- `spec.md` — 产品规格
- `features.json` — 特性列表（JSON 格式，防止 Agent 随意修改）
- `claude-progress.txt` — 跨会话的进度记录
...
- 实现迭代循环：Generator 实现 → Evaluator 验证 → 反馈 → Generator 改进
- 循环保护：最大迭代次数（如 10 轮），超出后暂停并询问用户
- 防止过早完成：Evaluator 必须验证所有特性，不可删除或编辑测试
...
- OpenTelemetry 集成：标准化追踪协议
- AgentOps 监控：跨 Agent 的执行追踪
- 审计日志：记录所有工具调用和决策
- 成本追踪：token 消耗、API 调用统计

#### 2B.7 Agent 间通信与任务分发

- **通信机制**：
  - 通过文件传递状态（不是内存共享）
  - 每个 Agent 完全重置上下文窗口（不是压缩）
  - 文件作为"合同"，明确输入输出格式

- **任务分发**：
  - 主 Agent（Coordinator）负责任务拆解和分配
  - 根据子任务特性选择合适的 Agent 类型
  - 支持串行执行（有依赖）和并行执行（无依赖）
  - 动态调整：根据执行情况重新分配任务

- **Sub-Agent 生命周期管理**：
  - 创建：主 Agent 根据需要动态创建子 Agent
  - 监控：实时追踪子 Agent 执行状态和资源消耗
  - 结果聚合：收集各子 Agent 的输出，整合为最终结果
  - 异常处理：子 Agent 失败时的重试、降级、回滚策略
  - 清理：任务完成后释放子 Agent 资源
...
- **复杂项目开发**：Planner 设计架构 → 多个 Generator 并行编写模块 → Evaluator 运行测试
- **深度调研**：多个 Researcher 并行搜索不同来源 → 主 Agent 聚合整理 → 生成结构化报告
- **大规模重构**：Planner 分析依赖 → 多个 Generator 并行修改文件 → Evaluator 验证构建和测试
- **内容创作**：Researcher 收集素材 → Generator 撰写初稿 → Evaluator 检查质量

---

## Phase 3 — 工具生态扩展

在 2A Agent 基础能力 + MCP 协议之上，接入更多工具能力。
...
| 优先级 | 功能 | 理由 |
|--------|------|------|
| **P0** | **2A.0 Agent Runtime 运行时基础设施** | 单 Agent 的容器，所有基础能力挂载于此 |
| **P0** | **2A.1 Tool Use 工具调用引擎** | Agent 架构的地基，所有后续能力依赖此 |
| **P0** | **2A.4 Agentic Loop 自主多步执行** | Agent 的核心循环，有了它才能"自主干活" |
| **P0** | 1.1 RAG 知识库问答 | 核心差异化能力，Supabase 基础已就绪 |
| **P0** | 1.2 对话持久化 | 基本可用性保障 |
| **P1** | **2A.2 文件系统操作工具** | Agent 最常用的工具，读写文件是基本功 |
| **P1** | **2A.3 终端命令执行工具** | 让 Agent 能跑代码、跑脚本、跑测试 |
| **P1** | **2A.5 规划与反思能力** | 复杂任务的可靠性保障 |
| **P1** | **2A.6 记忆系统** | 跨会话记住用户偏好和项目上下文，越用越懂你 |
| **P1** | **2A.7 MCP 协议支持** | 标准化扩展接口，即插即用接入海量外部工具 |
| **P1** | 1.3 输入体验升级 | 低成本高收益 |
| **P1** | 1.4 Markdown 渲染增强 | 代码复制等高频需求 |
| **P2** | **2A.8 工具组装应用场景** | 形成可复用的 Agent 应用模式 |
| **P2** | **2A.9 工具 Profile 过滤** | 工具多了之后的必需品 |
| **P2** | 3.1 Skill 技能系统 | 高频操作快捷化，扩展性基石 |
| **P2** | 3.2 Plugin 开放架构 | 让社区给 Agent 写功能 |
| **P2** | 3.4 定时任务与自动化 | 被动问答升级为主动服务 |
| **P2** | 5.1-5.3 Context Engineering | 长对话质量与成本的平衡 |
| **P2** | 3.5 文件上传与解析 | 扩展输入方式 |
| **P2** | 3.6 联网搜索 | 突破模型知识截止限制 |
| **P2/P3** | **2B Harness 多 Agent 协作架构** | 复杂、长期运行任务的终极方案，建立在 2A 之上 |
| **P3** | 3.3 Channel（飞书/微信/Slack） | 让 Agent 无处不在 |
| **P3** | 6.1 权限 + Hook 管线 | 多用户安全保障 |
| **P3** | 6.4 多模型路由 + Config 系统 | 生产化基础 |
| **P3** | Phase 4 知识管理体系 | 系统化知识管理 |
| **P3** | 5.4-5.7 高级交互 | 体验锦上添花 |
| **P4** | 6.5 部署与运维 | 按需推进 |
