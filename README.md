...
- **MCP 接入（可选）**：通过 Streamable HTTP 加载外部工具，支持用户/工具允许列表、参数验证、调用确认和请求级连接清理，配置方式见 [MCP 文档](docs/mcp.md)。
...
侧边栏 **连接 → MCP** 支持添加服务、测试连接、发现和选择工具、启停及删除，配置按用户保存。“英语、围棋、金融、编码、研报”等定制助手尚未接入完整业务实现。
