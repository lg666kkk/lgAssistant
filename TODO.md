...
- [ ] **RAG-05 增量同步**
  ...
  - chunk 级 hash：只重算变化 chunk 的 embedding
...
- [ ] **RAG-11 Reranker 精排**
  - 第一阶段召回 20-50 条候选
  - 第二阶段用轻量 LLM 或专门 reranker 给 query/chunk 相关性打分
  - 返回时保留 `vector_score`、`keyword_score`、`rerank_score`
  - trace 中展示重排前后顺序

- [ ] **RAG-12 Query 改写**
  - 支持 HyDE：生成假设答案再检索
  - 支持 multi-query：同一问题生成 2-3 个检索 query
  - 支持复杂问题子问题分解
...
- [ ] **RAG-16 知识库管理界面**
  ...
  - 支持手动同步/重建索引
  - 支持查看某页面的 chunk 和 embedding 元数据
